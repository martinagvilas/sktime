...
def set_classifier(cls, resampleId):
    ...
    elif cls.lower() in ['ee', 'elasticensemble']:
    ...
    if not overwrite:
        ...
        if train_file:
            ...
            if os.path.exists(full_path):
                ...
                train_file = False
        if train_file == False and not build_test:
            return
...
def write_results_to_uea_format(output_path, classifier_name, dataset_name, actual_class_vals,
    ...
    if split in ['TRAIN', 'train']:
        train_or_test = "train"
    elif split in ['TEST', 'test']:
        train_or_test = "test"
    else:
        raise ValueError("Unknown 'split' value - should be TRAIN/train or TEST/test")

    with open(str(output_path)+"/"+str(classifier_name)+"/Predictions/" + str(dataset_name) +
                "/"+str(train_or_test)+"Fold"+str(resample_seed)+".csv", "w") as file:
        ...
        for i in range(len(predicted_class_vals)):
