...
class ElasticEnsemblePostProcess:
    ...
    def __init__(self, results_path, dataset_name, distance_measures='all', resample_id=0, alpha=1):
        ...
        if distance_measures == 'all':
            self.distance_measures=['dtw','ddtw','wdtw','wddtw','lcss','erp','msm']
        else:
        ...
        for c_id in range(len(self.distance_measures)):
            file_path = self.results_path+self.distance_measures[c_id]+"/Predictions/"+self.dataset_name+"/trainFold"+str(self.resample_id)+".csv"
            with open(file_path, 'r') as f:
                ...
                if class_vals is None:
                    ...
                    raise ValueError("Class value mismatch when loading train file for "
                                     + str(self.distance_measures[c_id])+" and "+self.dataset_name)
                elif this_num_ins != num_ins:
                    raise ValueError("Inconsistent number of predictions in constituent training files: first spotted "
                                     "in train file for "+str(self.distance_measures[c_id])+" and "+self.dataset_name)
                ...
                for i in range(num_ins):
                    ...
                    for c in range(num_classes):
                        this_dists[i][c] = np.power(float(split_line[c+3]),self.alpha)*self.train_accs_by_classifier[c_id]
                if self.actual_train_class_vals is None:
                    ...
                    raise ValueError("Class values in files no not match for train - first spotted for "+str(self.distance_measures[c_id]))
        ...
        self.ee_train_dists = np.divide(self.ee_train_dists,sum(self.train_accs_by_classifier))

        # load test
        num_test_ins = None
        for c_id in range(len(self.distance_measures)):
            file_path = self.results_path + self.distance_measures[c_id] + "/Predictions/" + self.dataset_name + "/testFold" + str(self.resample_id) + ".csv"
            with open(file_path, 'r') as f:
                lines = f.readlines()
                third_line = lines[2].split(",")
                this_class_vals = third_line[-1].strip().replace("[", "").replace("]", "").split(" ")
                ...
                if this_class_vals != class_vals:
                    raise ValueError("Class value mismatch when loading test file for "
                                     + str(self.distance_measures[c_id])+" and "+self.dataset_name)
                if num_test_ins is None:
                    num_test_ins = this_num_ins
                elif num_test_ins != this_num_ins:
                    raise ValueError("Inconsistent number of predictions in constituent test files: first spotted "
                                     "in train file for " + str(self.distance_measures[c_id]) + " and " + self.dataset_name)
                this_dists = np.empty((num_ins, num_classes))
                ...
                if self.actual_test_class_vals is None:
                    ...
                    raise ValueError("Class values in files no not match for test - first spotted for "+str(self.distance_measures[c_id]))
        ...
        self.ee_test_dists = np.divide(self.ee_test_dists, sum(self.train_accs_by_classifier))

    def write_files(self, output_results_path, output_classifier_name="EE", write_train=True, write_test=True, overwrite=False):
        """
        ...
        if write_train is False and write_test is False:
            print("Train and test writing both set to false - method will terminate without doing anything")
            return

        if not overwrite:
            if write_train:
                full_path = str(output_results_path) + "/" + str(output_classifier_name) + "/Predictions/" + str(self.dataset_name) + "/trainFold" + str(self.resample_id) + ".csv"
                if os.path.exists(full_path):
                    print(full_path + " already exists and overwrite set to false, not writing Train", Warning)
                    write_train = False

            if write_test is True:
                full_path = str(output_results_path) + "/" + str(output_classifier_name) + "/Predictions/" + str(self.dataset_name) + "/testFold" + str(self.resample_id) + ".csv"
                if os.path.exists(full_path):
                    print(full_path + " already exists and overwrite set to false, not writing Test")
                    write_test = False

        if write_train is False and write_test is False:
            print("Train and test files both already exist and overwrite set to false - method will terminate without doing anything")
            return
        ...
        if write_train:
            ...
            third = str(acc) + ",NA,NA,-1,-1," + str(len(self.classes_)) + "," + str(self.classes_)
            write_results_to_uea_format(second_line=second, third_line=third, output_path=output_results_path, classifier_name=output_classifier_name, resample_seed=self.resample_id,
                                        predicted_class_vals=train_preds, actual_probas=train_probs, dataset_name=self.dataset_name, actual_class_vals=self.actual_train_class_vals, split='TRAIN')
        ...
        if write_test:
            ...
            third = str(acc) + ",NA,NA,-1,-1," + str(len(self.classes_)) + "," + str(self.classes_)
            write_results_to_uea_format(second_line=second, third_line=third, output_path=output_results_path, classifier_name=output_classifier_name, resample_seed=self.resample_id,
                                        predicted_class_vals=test_preds, actual_probas=test_probs, dataset_name=self.dataset_name, actual_class_vals=self.actual_test_class_vals, split='TEST')
