...
class Orchestrator:
    ...
    def _iter(self):
        ...
        for task, dataset in zip(self.tasks, self.datasets):
            ...
            for strategy in self.strategies:
                ...
                for cv_fold, (train_idx, test_idx) in enumerate(
                        self.cv.split(data, y)):
                    # for each fold, clone strategy to avoid updating
                    # already fitted strategies
                    strategy = clone(strategy)

                    yield (task, dataset, data, strategy, cv_fold,
                           train_idx, test_idx)

    def fit(self, overwrite_fitted_strategies=False, verbose=False):
        """Fit strategies on datasets"""

        for task, dataset, data, strategy, cv_fold, train_idx, test_idx in \
                self._iter():

            # skip strategy, if overwrite is set to False and fitted
            # strategy already exists
            if not overwrite_fitted_strategies and \
                    self.results.check_fitted_strategy_exists(
    ...
    def fit_predict(self,
        ...
        for task, dataset, data, strategy, cv_fold, train_idx, test_idx in \
            ...
            if not overwrite_predictions and test_pred_exist and (
                    train_pred_exist or not predict_on_train) and \
                    not overwrite_fitted_strategies and (
                    fitted_stategy_exists or not save_fitted_strategies):
                print(
                    f"Skipping strategy: {strategy.name} on CV-fold: "
                    f"{cv_fold} of dataset: {dataset.name}")
                continue
    ...
    def _validate_tasks_and_datasets(tasks, datasets):
        ...
        if not isinstance(datasets, list):
            raise ValueError(
                f"datasets must be a list, but found: {type(datasets)}")
