...
def stomp_ab(ts1, ts2, m):
    ...
    dp = calculate_distance_profile(dot_prod, ts1_mean[0], ts1_std[0],
                                    ts2_mean, ts2_std, m, n_ts2_subs)
    ...
    for i in range(1, n_ts1_subs):
        for j in range(n_ts2_subs - 1, 0, -1):
            dot_prod[j] = dot_prod[j - 1] - ts2[j - 1] * ts1[i - 1] + ts2[
                j - 1 + m] * ts1[
                              i - 1 + m]  # compute the next dot products
            # using the previous ones
        dot_prod[0] = first_dot_prod[i]
        dp = calculate_distance_profile(dot_prod, ts1_mean[i], ts1_std[i],
                                        ts2_mean, ts2_std, m, n_ts2_subs)
        mp[i] = np.amin(dp)
        ip[i] = np.argmin(dp)
