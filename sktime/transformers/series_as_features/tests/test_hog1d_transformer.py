...
def test_bad_num_intervals(bad_num_intervals):
    ...
    if not isinstance(bad_num_intervals, int):
        with pytest.raises(TypeError):
            HOG1DTransformer(num_intervals=bad_num_intervals) \
                            .fit(X).transform(X)
    else:
...
def test_output_of_transformer():
    ...
    res = h.transform(X)
    orig = convert_list_to_dataframe([[0, 0, 0, 0, 4,
                                       0, 0, 0, 0, 0,
                                       0, 4, 0, 0, 0, 0]])
    orig.columns = X.columns
    assert check_if_dataframes_are_equal(res, orig)

    X = generate_df_from_array(np.array([-5, 2.5, 1, 3, 10,
                                         -1.5, 6, 12, -3, 0.2]),
                               n_rows=1, n_cols=1)
    h = h.fit(X)
    res = h.transform(X)
...
@pytest.mark.parametrize("num_bins,corr_series_length", [(4, 8),
                                                         (8, 16),
                                                         (12, 24)])
...
def test_hog1d_performs_correcly_along_each_dim():
    ...
    h = HOG1DTransformer().fit(X)
    res = h.transform(X)
    orig = convert_list_to_dataframe([[0, 0, 0, 0, 4, 0, 0, 0, 0, 0,
                                       0, 4, 0, 0, 0, 0],
                                      [0, 0, 0, 0, 4, 0, 0, 0, 0, 0,
                                       0, 4, 0, 0, 0, 0]])
    orig.columns = X.columns
